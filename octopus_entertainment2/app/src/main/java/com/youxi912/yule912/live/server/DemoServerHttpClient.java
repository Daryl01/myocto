package com.youxi912.yule912.live.server;
...
import com.youxi912.yule912.live.config.DemoServers;
import com.youxi912.yule912.live.server.entity.AddVideoResponseEntity;
import com.youxi912.yule912.live.server.entity.RoomInfoEntity;
import com.youxi912.yule912.live.server.entity.TranscodeResponseEntity;
import com.youxi912.yule912.live.server.entity.VideoInfoEntity;
import com.youxi912.yule912.live.util.sys.AndTools;
    ...
    public void anchorLeave(String roomId, final DemoServerHttpCallback<Void> callback) {
        ...
        NimHttpClient.getInstance().execute(url, headers, body.toString(), new NimHttpClient.NimHttpCallback() {
            ...
            public void onResponse(String response, int code, Throwable exception) {
                if (code != 200 || exception != null) {
                    LogUtil.e(TAG, "register failed : code = " + code + ", errorMsg = "
                            + (exception != null ? Utils.checkNotNull(exception.getMessage()) : "null"));
                    callback.onFailed(code, (exception != null ? Utils.checkNotNull(exception.getMessage()) : "null"));
                    return;
                }
                try {
                    JSONObject resObj = JSONObject.parseObject(response);
